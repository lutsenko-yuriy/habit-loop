  ...
  Future<void> seedLocalPacts({int successPercent = 0}) async {
    ...
    try {
      ...
      for (final p in existingPacts) {
        // A null reminderOffset means no reminder was ever scheduled for this
        // pact (the invariant the edit flow also relies on) — skip the
        // cancellation round-trip entirely for the common case of deleting
        // freshly-seeded pacts, which always have a null offset (HAB-229).
        if (p.reminderOffset != null) {
