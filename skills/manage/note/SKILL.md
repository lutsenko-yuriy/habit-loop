...
**Set the `bookmarks:` frontmatter key.** Read `docs/knowledge/notes/BOOKMARKS.md` and pick the 1–3 existing bookmarks that best match the note text. If nothing in the vocabulary fits, use `bookmarks: []` — do not invent a new bookmark here (that stays a `/debrief`-time or `/checkup heavy`-time decision, since both have room for a multi-question dialog and `/note`'s one-question budget above does not). Format: an inline list at the very top of the file, e.g. `bookmarks: [ci-flakiness, scope-creep]` (block-list `- item` style is rejected by `index.py --check`). Merge into any existing `bookmarks:` key rather than overwriting it. **Stage the file this step wrote** (`git add` on whichever path was actually used — `docs/knowledge/notes/HAB-XX.md` or the `HAB-XX-adjust-N.md` fallback — plus `BOOKMARKS.md` if this step touched it) **before regenerating** — `index.py` only sees git-tracked or staged `.md` files (HAB-250); an unstaged new note file would be silently excluded from the freshly-generated `INDEX.md`, and the very next commit that tracks it would then make CI's `--check` fail as newly-stale. Then run `python3 scripts/notes/index.py` to regenerate `INDEX.md`, and **stage the regenerated `INDEX.md` too** (`git add docs/knowledge/notes/INDEX.md`) — it's written after the `git add` above, so it's left modified-but-unstaged unless added explicitly.

**Commit the staged file(s), scoped to this directory** (`git commit -m "docs: note for HAB-XX" -- docs/knowledge/notes/`) — `/note` has no later step of its own to do this, so leaving the note merely staged risks it being silently swept into whatever unrelated commit runs next on this branch (the same failure shape HAB-250 fixed, from a different angle). The `--` pathspec keeps this commit to the knowledge-base files even if other work (e.g. a mid-TDD-cycle change) happens to be staged at the same time — `/note`'s own constraints forbid touching anything outside `docs/knowledge/notes/`. Do not push — that stays with whatever commits the rest of the current work.
