...
**Set the `bookmarks:` frontmatter key.** Pick the 1–3 bookmarks from `docs/knowledge/notes/BOOKMARKS.md` (plus any new one approved in step 4) that best match the debrief. If nothing fits, use `bookmarks: []`. Merge into any existing `bookmarks:` key rather than overwriting it. If step 4 approved a new bookmark, add it to `docs/knowledge/notes/BOOKMARKS.md` as part of this same file set. Then run `python3 scripts/notes/index.py` to regenerate `INDEX.md`.
