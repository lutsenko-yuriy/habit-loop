...
- WU0 is always the first unit. For tickets with a user-facing flow `draft-scenarios` can assert against via `AppHarness`: integration scenarios, committed as a `[test]`-tagged PR before any production code lands. For everything else: WU0 is a **verification checklist** — concrete, ticket-specific checks derived from what the ticket is meant to do. Mark each item **[agent]** (the agent runs it itself — dry-run a skill and diff output, grep for a pattern, count tool calls, re-run a script) or **[human]** (only a person can judge it — visual/subjective review, a live device, an external service). Write it into the plan's Test Strategy section instead of a WU0 table row. `implement` runs every [agent] item and asks the user to confirm every [human] item before the final WU's PR opens — see `skills/build/implement/SKILL.md`.
