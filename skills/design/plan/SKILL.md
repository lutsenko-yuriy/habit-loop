...
- If a design choice in this plan would benefit from external validation (precedent from other apps/tools, a published guideline, an established best practice) and the ticket isn't genuinely novel (no comparable precedent exists to research), propose an optional **Research-WU** as the very first unit — runs `/research` (or an equivalent bounded, cited check), writes findings to the ticket's knowledge note, no separate branch or PR.
- A **Scenario-WU** or **Checklist-WU** is always the next unit (first, if no Research-WU). For tickets with a user-facing flow `draft-scenarios` can assert against via `AppHarness`: integration scenarios, committed as a `[test]`-tagged PR before any production code lands — this is the Scenario-WU. For everything else: a **Checklist-WU** — a verification checklist of concrete, ticket-specific checks derived from what the ticket is meant to do. Mark each item **[agent]** (the agent runs it itself — dry-run a skill and diff output, grep for a pattern, count tool calls, re-run a script) or **[human]** (only a person can judge it — visual/subjective review, a live device, an external service). Write it into the plan's Test Strategy section instead of a WU0 table row. `implement` runs every [agent] item and asks the user to confirm every [human] item before the final WU's PR opens — see `skills/build/implement/SKILL.md`.
- Research-WU, Scenario-WU, and Checklist-WU are named, not numbered — pre-implementation units that never carry production code. Every WU after them keeps the existing numeric naming (WU1, WU2, …) in the WU table, unchanged.
