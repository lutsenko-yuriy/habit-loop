...
Widget _buildApp(
  ...
  VoidCallback? onAdjustAndStartAgain,
}) {
  return MaterialApp(
...
void main() {
  group('PactDetailPageIos – note section visibility', () {
    testWidgets('shows note field for a stopped pact', (tester) async {
      ...
      // the multi-line note field below has its own internal EditableText
      // Scrollable, which otherwise makes that lookup ambiguous.
  ...
  group('PactDetailPageIos — adjust and start again (HAB-202)', () {
    testWidgets('shows Adjust and start again button on stopped pact with no successor when chaining is enabled',
        (tester) async {
      final state = PactDetailState(pact: _stoppedPact, stats: _stats, isLoading: false);
      await tester.pumpWidget(_buildApp(state, pactChainingEnabled: true, onAdjustAndStartAgain: () {}));
      await tester.pumpAndSettle();
      await tester.scrollUntilVisible(
    ...
    testWidgets('hides Adjust and start again button when a successor already exists, shows Next Pact instead',
      ...
      final state = PactDetailState(pact: _stoppedPact, stats: _stats, isLoading: false, successorPact: _successorPact);
      await tester.pumpWidget(
        _buildApp(state, pactChainingEnabled: true, onOpenNextPact: () {}, onAdjustAndStartAgain: () {}),
      );
      await tester.pumpAndSettle();
      await tester.scrollUntilVisible(
    ...
    testWidgets('hides Adjust and start again button when pactChainingEnabled is false', (tester) async {
      final state = PactDetailState(pact: _stoppedPact, stats: _stats, isLoading: false);
      await tester.pumpWidget(_buildApp(state, onAdjustAndStartAgain: () {}));
      await tester.pumpAndSettle();
    ...
    testWidgets('hides Adjust and start again button when onAdjustAndStartAgain is null', (tester) async {
      final state = PactDetailState(pact: _stoppedPact, stats: _stats, isLoading: false);
      await tester.pumpWidget(_buildApp(state, pactChainingEnabled: true));
      await tester.pumpAndSettle();
    ...
    testWidgets('tapping Adjust and start again button calls onAdjustAndStartAgain', (tester) async {
      ...
      final state = PactDetailState(pact: _stoppedPact, stats: _stats, isLoading: false);
      await tester.pumpWidget(
        _buildApp(state, pactChainingEnabled: true, onAdjustAndStartAgain: () => tapped = true),
      );
      await tester.pumpAndSettle();
      await tester.scrollUntilVisible(
        find.byKey(const Key('pact-detail-adjust-and-start-again-button')),
