...
def read_frontmatter(skill_path: str):
    """Return (effort, reasoning, needs_session_tools, context, render_html_view, tools, max_turns, body)."""
    from .constants import DEFAULT_MAX_TURNS as MAX_TOOL_TURNS
    text = Path(skill_path).read_text()
    m = re.match(r"^---\n(.*?)\n---\n", text, re.DOTALL)
    if not m:
        return None, None, False, None, False, [], MAX_TOOL_TURNS, text
    fm = m.group(1)
    ...
    # Opt-in only — most context: linear skills (e.g. draft-scenarios) must not
    # trigger the backlog.local.html side effect (HAB-222). Only summarize sets it.
    render_html_view = bool(re.search(r"^render_html_view:\s*true", fm, re.MULTILINE))
