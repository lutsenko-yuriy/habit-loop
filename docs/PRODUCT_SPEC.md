...
- Mark a showup done hands-free via Siri, without unlocking the phone (iOS only; feature flag: `voice_mark_done_enabled`)
  - "Which showups should I do today?" reads back today's remaining showups — not yet done, still-pending or auto-failed — starting with the next upcoming one. A pending showup covered by an active break is excluded
  - "Mark \<habit\> done" only marks, never fails, a currently *pending* showup by voice (auto-failed showups can be heard in the query above but not marked done this way — redemption needs a written note, in-app only):
  ...
  - Not available: Android/Google Assistant, marking a showup failed by voice, or belonging to a stopped/completed pact
