...
The app follows [Semantic Versioning](https://semver.org/) with the Flutter version format `X.Y.Z+buildNumber` in `pubspec.yaml`. For the GitHub Actions job graph, manual-dispatch inputs, TestFlight distribution, and secrets/variables reference, see `docs/CI_PIPELINE.md`.
