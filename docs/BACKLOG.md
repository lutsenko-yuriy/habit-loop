...
- [HAB-14](https://linear.app/iurii-lutsenkos-workspace/issue/HAB-14/tech-debt-lazy-showup-generation) **Tech debt: lazy showup generation** — Showups are currently generated eagerly for the entire pact duration at creation time. Should be revisited when notifications are implemented: generate lazily (rolling window ahead of today) to avoid scheduling thousands of notifications upfront and keep the repository lean. (Tech Debt)
