...
- [app] HAB-254 (WU3, final): wired `NotificationReconciliationService.reconcile()` to two triggers via `AppLifecycleReconciler` (`lib/slices/reminder/ui/generic/`, wrapping the app tree in `main.dart`) — app foreground and a new `SyncService.pullCompleted` broadcast stream (emitted by `FirestoreSyncService` at the end of every non-aborted `pullRemoteChanges()`). A shared `ReconciliationThrottle` (re-entrancy lock + 30s minimum interval) keeps the two triggers from overlapping or thrashing. Notifications now self-heal drift (stale on-break state, missed reschedules) automatically instead of only on the next explicit scheduling call.
