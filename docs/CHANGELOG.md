...
- [wip] HAB-195 WU4.1 polish: Separated "Take a Break"/"Stop Pact" from the read-only sections above with a divider on Pact Detail (iOS + Android), and smoothed the break-creation screen's "No end date yet" toggle so the toggle shifts position with an animated resize instead of jumping, while the end-date row still fades its opacity in sync.
