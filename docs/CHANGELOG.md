...
- [meta] HAB-274: backported harness fixes surfaced retrofitting mordovorot onto the `yuriys-agentic-boyz` template (also fixed upstream, YAB PR #13) — new `skills/shared/git-cli-safety.md` fragment (heredoc/backtick mangling in `git`/`gh` commands) referenced from `review-architecture`, `audit-code`, and `ship`; `calibrate`'s stub-formats resource gets a placeholder-check step before spawning; `calibrate` itself now cross-checks `docs/MODEL_TIERS.md`'s capability map against `skills/*/*/SKILL.md` on disk; `MULTI_WU.md` documents the `[Unreleased]` target for intermediate-WU `[wip]` entries and calls out the final-WU debrief-before-merge approval-order inversion explicitly.
- [meta] HAB-276: adopted ADR-0010 — sync template updates via plain-git diff-and-apply workflow (Copier's core idea, no new dependency), scoped to template-owned paths with agent-driven conflict resolution. Strategy evaluated against `docs/CONSTRAINTS.md`; precedent surveyed (git subtree/submodule, Copier, cruft, Yeoman, framework-native patterns).
...
- [ci] HAB-205: added a hard 20-minute wrapper-level `timeout -k 1m` around the `flutter test` invocation for integration scenarios (CI's `run-scenarios` composite action and the local `/run-scenarios` skill, with a `gtimeout` fallback for macOS) — the test framework's own `--timeout=300s` bounds a single test, not the whole process, so a genuinely stuck run (e.g. a runaway fixture date range, seen in HAB-202) previously sailed past it silently instead of failing fast with a clear signal. A killed run also now skips the scenarios badge update, so a timeout can't publish a misleadingly green badge from truncated results.
...
- [test] HAB-259: fixed a latent time bomb in `pact_detail_view_model_test.dart` — its shared `_pact` fixture hardcoded `endDate: DateTime(2026, 9, 1)` with no pinned "now", so once wall-clock time caught up to that date the pact silently auto-completed mid-test (breaking two tests that assumed it stayed active; several others kept passing while silently running against the wrong pact status). Pinned `pactDetailNowProvider` as a default override inside the file's shared `_makeContainer` helper, covering every call site at once. No production code touched.
...
- [wip] HAB-232 (WU7): Personalized notification titles — reminder, deadline, hurry-up, and welcome-back notifications now include the user's first name when one is on file. Flips `display_name_personalization_enabled` to `true` (release-gated to 0.59.0 via `FeatureFlags`). Removed dormant EXP-001 (notification-text-urgency) experiment code and RC dispatch (PR #425). No user-visible change yet (flag stays off in 0.58.0).
...
- [test] HAB-260 (WU1): `FakeRemoteConfigService.withTestDefaults(...)` — a new factory that layers a shared `TestRemoteConfigDefaults.all` map underneath any caller-supplied `overrides`, so a future RC flag-default flip no longer needs a grep-and-patch sweep across every test construction site. The bare constructor is untouched and still returns production defaults. No production code touched.
- [test] HAB-260 (WU2): `AppHarness.create` now takes a `remoteConfigOverrides` map instead of a whole `FakeRemoteConfigService`, letting it merge that map into the shared test defaults before constructing the service. All call sites in `integration_test/` updated to pass a map instead (PR #433). No production code touched.
- [test] HAB-260 (WU3, final): migrated the remaining `integration_test/` call sites with inline `extraOverrides` (about screen, create pact, dashboard kebab menu, notification reconciliation, break timeline, pact timeline flows) to the new `remoteConfigOverrides` map, and added a guard test asserting nothing under `integration_test/` overrides `remoteConfigServiceProvider` directly, so the old footgun pattern can't silently return (PR #434). WU4 (optional widget-test cleanup) dropped per plan. No production code touched.
...
- [wip] HAB-220 (WU2): extracted CI/CD pipeline mechanics (job graph, manual-dispatch inputs, granular dispatch workflows, TestFlight distribution/What's-New, secrets/variables tables) out of `docs/VERSIONING.md` into a new `docs/CI_PIPELINE.md`; `VERSIONING.md` now covers only versioning rules (semver, build numbers, CHANGELOG tags, release-note content rules). Updated cross-references in `AGENTS.md`, `docs/workflows/FEATURE.md`, and `README.md`.
- [wip] HAB-220 (WU3): `AGENTS.md` now only unconditionally `@`-includes `docs/workflows/FEATURE.md`; `TROUBLESHOOT.md`/`RESEARCH.md`/`POSTMORTEM.md` are read on-demand by their trigger condition instead of loaded every session. Extracted the Multi-WU appendix (absorbing `skills/shared/wu-splitting-guidelines.md`) into a new `docs/workflows/MULTI_WU.md`, linked from `FEATURE.md` rather than inlined.
- [meta] (PR #407) HAB-220 (WU4, final): de-duplicated the In-QA path-pattern list (single-sourced to `skills/shared/project-config.md`) and the CHANGELOG tag taxonomy (single-sourced to `docs/VERSIONING.md`). Trimmed redundant explanations from `ship/SKILL.md` and `changelog-tags.md`, collapsed `changelog-tags.md`'s tag-taxonomy table to a bullet-syntax reference. Completed the Checklist-WU with ≈38% fixed token cost reduction.
- [test] HAB-254 (Scenario-WU): integration scenario stubs for the notification reconciliation sync mechanism (`integration_test/notification_reconciliation_flow_test.dart`) — break-elapsed restore, active-break non-interference, sync-pull trigger, idempotence, and kill-switch coverage. No driver code yet.
- [wip] HAB-254 (WU1): extracted `ReminderPlanner` — a pure desired-set computation — out of `ReminderSchedulingService` (`lib/slices/reminder/application/reminder_planner.dart`, `planned_reminder.dart`), behaviour-preserving. `ReminderSchedulingService` now resolves locale/remote-config context, delegates to the planner, then applies the plan and logs analytics. No functional change; sets up WU2's reconciliation diff to reuse the same planning logic.
- [wip] HAB-254 (WU2): added `NotificationReconciliationService` (`lib/slices/reminder/application/notification_reconciliation_service.dart`) — loads active pacts/showups/breaks, computes the desired notification set via `ReminderPlanner`, diffs it against `NotificationService.getPendingNotifications()` by id, and schedules whichever notification kind is missing / cancels a showup only when it's genuinely no longer eligible for any notification (done/failed or now on-break) — a showup merely in progress keeps its pending deadline/hurry-up. iOS's 64-notification budget is now shared across pacts in one run instead of each pact independently claiming 64. Added the `notification_reconciliation_enabled` kill-switch (default `true`) and the `notifications_reconciled` analytics event, logged only when a run actually changes something. Extracted `ReminderLocaleResolver` so locale resolution has one shared path (`ReminderSchedulingService` also updated to use it). Wired a provider (`notificationReconciliationServiceProvider`); no trigger calls `reconcile()` yet — that's WU3.
- [wip] HAB-232 (WU4): EnterNamePage (skippable first-launch page for fresh installs), isNameEntryShown onboarding preference flag, display_name_entry_completed analytics event, enter_name screen view (PR #420).
- [wip] HAB-232 (WU5): "Change name" menu entry and dialog (iOS + Android), display_name_changed analytics event, change_name screen view (PR #421).
- [wip] HAB-232 (WU6): personalized onboarding slide 0 title and a new dashboard greeting header, both reading `personalizedNameProvider` and falling back to existing neutral copy when no name is on file. No user-visible change yet (flag still defaults off until WU7).
...
- [app] HAB-249: debug RC overrides screen now shows a flag's minimum app version (e.g. "since 0.53.0") when one is registered in `RemoteConfigDefaults.releaseVersions` — surfaced live while device-testing HAB-246's flag flip.
...
- [meta] (PR #393) HAB-240 (research-only, debrief): release-batching research — solo dev + AI agents vs. hypothetical small team, Linear Milestone/Project/Cycle feature fit, impact on existing `[Unreleased]` batching model. Findings: current per-PR `/ship` model is solid for solo dev; small-team scale adds friction (release notes aggregation, duplicate version-bump safety check, partial-merge handling) addressable via a batching layer, but no precedent found that justifies building one today. ADR-0005 captures the decision to stay with per-PR releases for now.
- [ci] HAB-239: `scripts/design_system/lint.py` also enforces `AppTypography` token usage — flags a raw `TextStyle(...)` literal whenever its properties (fontSize/fontWeight/fontStyle/letterSpacing/height/fontFamily) exactly duplicate an existing token, so a call site that should be sharing a token can't silently drift back to a hand-rolled duplicate. Deliberately narrower than the spacing check: `AppTypography`'s own doc comment calls out that genuinely one-off styles are meant to stay raw, so only exact token duplicates are flagged, not every raw literal. Caught and fixed 3 pre-existing duplicates (`emphasis`, `caption`, `valueEmphasis`) as part of landing a clean baseline. Same CI wiring point as HAB-189 (`.github/actions/run-tests/action.yml`), same `typography_allowlist.txt` suppression convention.
- [ci] (PR #387) HAB-189: `scripts/design_system/lint.py` enforces `AppSpacing` token usage in `EdgeInsets`/`SizedBox`/`Padding` constructors at CI lint time — blocks raw numeric spacing literals (with allowlist exceptions for fixed dimensions like icon sizes). Wired into `.github/actions/run-tests/action.yml` alongside existing CHANGELOG and notes-index checks. Prevents backsliding on HAB-187's one-time spacing sweep to tokens.
...
- [meta] (PR #378) HAB-222 (WU2, final): opt-in HTML backlog view wired into `summarize`. `LinearProvider.format_context` stays pure markdown; new `render_backlog_view` method (gated by `render_html_view: true` frontmatter flag) renders/writes/opens a sortable HTML table (`backlog.local.html`) as a side effect — only `summarize` sets the flag, so `draft-scenarios` (which also declares `context: linear`) is unaffected. `docs/ARCHITECTURE.md` and `skills/manage/summarize/SKILL.md` updated. Review caught shared-method side effect leaking to all `context: linear` callers, silently-swallowed misconfiguration (now warns to stderr), and doc/code drift.
- [wip] (PR #376) HAB-222 (WU1): new `scripts/skill_router/providers/linear/html_view.py` renders the Linear backlog as a self-contained, sortable HTML table (`render_backlog_html`/`write_backlog_html`/`open_in_browser`) — fully tested but intentionally unwired; `/summarize` behaviour is unchanged until WU2 wires it in. `ISSUES_QUERY` gains a `url` field (per-issue link for the `#` column) and a fix for a filter bug found live: the `nin` exclusion listed `"cancelled"` (British spelling) instead of Linear's actual `"canceled"` enum value, so canceled/abandoned issues were never actually excluded from any backlog fetch — 8 of 34 issues in one live run. `first_line()` and a new `active_milestone()` extracted from `context.py` so the chat and HTML renderers share truncation/milestone-selection logic instead of drifting. `backlog.local.html` added to `.gitignore` ahead of WU2's wiring.
- [wip] (PR #373) HAB-221 (WU2): one-time corpus backfill of `docs/knowledge/notes/` — a single read-through of all 65 existing notes produced a 21-term emergent bookmark vocabulary (`BOOKMARKS.md`), applied to every note's frontmatter via `scripts/notes/index.py --apply-bookmarks`. Top recurring themes: `debugging-methodology`, `multi-wu-scope`, `planning-gaps` (10 notes each), `ci-flakiness` (9). `INDEX.md` regenerated.
- [wip] (PR #371) HAB-221 (WU1): new `scripts/notes/index.py` generates and validates a tagged table of contents (`INDEX.md`) over `docs/knowledge/notes/` — each note's `bookmarks:` frontmatter is checked against a hand-authored `BOOKMARKS.md` allow-list, distinguishing a missing key (not yet reviewed) from an explicit `bookmarks: []` (reviewed, nothing noteworthy). `--apply-bookmarks` does the one-shot corpus backfill, landing in WU2.
- [meta] (PR #370) HAB-217: new `skills/manage/draft-release-notes/` skill (invoked automatically from `ship` step 2) drafts plain-language `[user]` CHANGELOG bullets against a 7-item checklist before technical entries are written — reduces back-and-forth on release notes by grounding bullets in the checklist (understandable, cared-about, no jargon, plain language, right altitude, currently-true, no promotional/CTA language). Includes `user-bullet-checklist.md` resource and `.claude/commands/` stub.
...
- [meta] (PR #368) HAB-176 (WU2): `summarize` drops its per-ticket `get_issue` loop (`PLAN.local.md` survey step) for a single `fields`-scoped `list_issues` call. `ship` collapses its two adjacent `get_issue` preconditions into one fetch. `brief`'s duplicate-ticket search now passes explicit `fields`/`includeArchived: false`. Highest-value WU of the ticket — est. ~30k+ tokens off a session start. (WU2 also deleted `summarize`'s live-MCP fallback branch entirely; WU3, below, restored it — see that bullet.)
- [meta] (PR #369) HAB-176 (WU3, final): `analyze`, `plan`, `draft-scenarios`, and `implement` now reuse an issue's full details or plan comment from the session transcript when a preceding step already fetched them this session, instead of unconditionally re-fetching — cuts the up-to six/seven full `get_issue` calls a single ticket's lifecycle could otherwise cost. Also restores `summarize`'s live `list_issues`/`list_milestones` fallback (and its product-vs-process classification) that WU2 had deleted outright — a broken `skill_router` (LM Studio down, missing env vars, or a script bug) was leaving sessions stuck with no backlog and no way around it.
- [meta] (PR #374) HAB-221 (WU3, final): write-time bookmarking added to `/note` and `/debrief` skills; quarterly re-bookmark dimension added to `/checkup heavy`; blocking `--check` step wired into CI (`run-tests` composite action) to prevent `docs/knowledge/notes/` corpus from decaying out of sync. Docs updated (`docs/knowledge/README.md`, `AGENTS.md`). Audits caught and fixed two real bugs: `TEMPLATE.md` missing fail-safe `bookmarks: []` default (would fail CI for any future note created via `/plan`), and `skill_router` config's kill-switch field having three fail-open holes (bare `python3` no `tomllib`, cwd-relative path, untyped string-vs-bool parsing). All three WUs are shipping (WU1 in #371, WU2 in #373, WU3 in #374) — related tickets HAB-223 (merge-race race on `INDEX.md`) and HAB-224 (hardware for local LLM) deferred for follow-up.
...
- [user] The pacts list's "Done"/"Stopped" counts now spell out how many are archived (e.g. "3 pacts done (2 archived)") instead of silently excluding archived pacts from the total.
- HAB-212: `archivedNote`'s base count now uses the full `doneCount`/`cancelledCount` rather than `count - archivedCount`; `pactsArchivedParenthetical`/`pactsAllArchived` l10n strings reworded to "X archived"/"all archived" across all 4 locales.
...
- [wip] HAB-206 (WU3): Chain-link navigation on `PactDetailScreen` now animates only the individual values that actually change — habit name, status badge, and each unconditionally-shown stat/timeline tile value — as a small combined slide + fade (250ms, ease-out), direction-aware. Section headers, buttons, dividers, and structurally-conditional rows (e.g. the "Remaining"/"Cancelled" stat cell, the stopped-date tile) stay static/instant rather than animating. New `AnimatedValueTransition` wraps each value; `PactDetailScreen` snapshots the outgoing pact's state for the duration of the transition and passes it down explicitly, so the outgoing/incoming comparison doesn't depend on Element/State identity surviving the swap. The content `ListView`'s own key now stays pinned to the screen's *original* entry pact id (previously the currently-displayed one) so its whole subtree — including `BreakBannerReveal`'s own reveal animation and the notes section — survives a chain-link swap instead of popping in/out; a small invisible marker exposes the currently-displayed pact id to integration scenarios instead. `PactDetailViewModel.load()` and `PactDetailCache` now remember resolved chain-neighbor identities as they're discovered (and warm one hop further in the background) so a chain-link swap along an already-visited branch resolves its own neighbors from memory instead of a fresh DB round trip — this is what actually keeps the swap free of a loading-spinner blink, not just the WU2 bundle prefetch. The `_isAnimating` guard from WU1 still stays set for the full visual transition, not just the data reload. `AnimatedValueTransition` adopts a new `previousChild` from both `initState` and `didUpdateWidget` — with the ListView subtree now stable, most instances survive a swap and only ever see the change via a rebuild, not a fresh Element. While a reload is in flight, the platform pages now render the outgoing pact's own already-loaded content (via `previousState`) instead of a spinner, closing the residual loading-blink gap regardless of cache timing. Widget- and unit-tested; the full `pact_chain_test.dart` integration suite verified passing live on an iOS simulator.
- [wip] HAB-206 (WU3 polish): The Remaining/Cancelled stat cell now crossfades label+value together on any chain-link swap where both pacts show one (previously gated on an unchanged label, which meant it almost never animated for the common active↔stopped hop). `StatusBadge` takes an optional fixed `width`, pinned on pact detail to the widest of the three status labels, so it no longer visibly jumps sideways as the Row relayouts around it growing then shrinking. The stopped-date tile, days-remaining tile, notes section, and archive section now all reveal/collapse via `AnimatedReveal` instead of popping in/out via a bare `if`; the end-date tile's own label ("Ends"/"Ended") now crossfades alongside its value. Root-caused the days-remaining tile rendering narrow and centered (and the notes section's earlier internal-scrolling regression): the `Column` wrapping each revealed section needs `crossAxisAlignment.stretch`, not just `mainAxisSize.min` — `Align` (inside `AnimatedReveal`) loosens the incoming width constraint, and without `stretch` a `min`-sized `Column` shrink-wraps to its content's own intrinsic width instead of filling the row. The archive section's header is explicitly left-aligned via `Align` rather than blanket `stretch`, since stretching would also grow its button to full width, away from its established small-centered-link style. The "Take a Break"/"Stop Pact" section (active-pact-only) gets the same `AnimatedReveal` treatment, `mainAxisSize.min` without `stretch` — same button-sizing rationale as the archive section.
- [wip] HAB-206 (WU3 review fixes): `AnimatedValueTransition` moved from the pact slice to `lib/theme/widgets/` and lost its pact-specific naming (`ChainNavigationDirection` → `SlideDirection`, `kChainNavigationTransitionDuration` → `kAnimatedValueTransitionDuration`) — it has no domain coupling, so it belongs alongside `AnimatedReveal` rather than only reachable from the pact slice. `PactDetailCache.evict()` now also clears the reverse `_successorByPredecessorId` index (previously only the forward `_pactsById` entry was cleared, leaving a stale successor identity reachable via `peekSuccessor` after an evicted pact's neighbor relationship changed). `AnimatedReveal` now ignores hit-testing on its child while collapsing (`visible` just went `false`, value still fading out) so a tap can't land on a lifecycle button (Stop pact, Save note, Archive) that's visually on its way out and act against pact data that's already stale by the time it lands.
- [wip] HAB-206 (WU2): `PactDetailViewModel.load()` now warms `PactDetailCache` for the predecessor/successor pact bundle as a fire-and-forget call, so a subsequent chain-link swap (WU1) hits a warm cache instead of a fresh DB round trip. Unit-tested only — no visible UI change.
- [wip] HAB-206 (WU1): `PactDetailScreen`'s "Previous Pact"/"Next Pact" links now swap the currently-displayed pact in place (`_currentPactId` internal state) instead of pushing a new route, so a single screen instance stays mounted no matter how far the user bounces through a chain — the system back button always exits directly to wherever the screen was originally opened from. Removed the now-unnecessary `cameFromPactId` pop/push disambiguation. Added an `_isAnimating` guard so a second chain-link tap mid-swap is a no-op (data reload today; the WU3 transition animation will extend this same guard). Makes WU0's in-place-swap stub scenarios green and deletes the WU0-restored legacy push/pop scenario group in the same commit — see `docs/knowledge/notes/HAB-206.md`.
...
- [wip] HAB-202 WU1: Added `Pact.predecessorPactId` (nullable, immutable after insert) through `Pact`, `PactMapper` (`toRow`/`fromRow` only), and `SyncMapper`. Schema migration v6 adds the `predecessor_pact_id` column plus a partial unique index (`idx_pacts_predecessor_pact_id ... WHERE predecessor_pact_id IS NOT NULL`) enforcing "at most one successor per pact" — no self-referential FK, so an out-of-order Firestore pull can't trigger a violation. Added `PactRepository.getSuccessor(pactId)` and a `savePact` successor-uniqueness guard to both `SqlitePactRepository` and `InMemoryPactRepository`. No UI yet.
- [wip] HAB-202 WU2: Added `PactChainService` (default versioned habit name via root-walk, prefilled `PactBuilder` for "Adjust and start again"). `PactBuilder.build` and `PactService.createPactFromBuilder` now accept `predecessorPactId`. `PactCreationState` gained `source`/`predecessorPactId`/`prefillBuilder`, seeded from a new `pactCreationConfigProvider` in `PactCreationViewModel.build()`; `submit()` persists the predecessor link and enriches `PactCreatedEvent` with `source`, `predecessor_pact_id`, and the `*_changed_from_*` diff flags. No UI entry point yet.
- [wip] HAB-202 WU3: Pact detail screen now shows "Previous Pact"/"Next Pact" links, gated behind a new `pact_chaining_enabled` kill-switch (defaults `false` during the WU3/WU4 rollout, flips to `true` at the end of WU4). Tapping a link pops back to an existing page instance instead of pushing a duplicate when bouncing between a predecessor and successor pact, making the two navigation-stack scenarios from WU0-scenario_correction green. Added `PactChainLinkTappedEvent` analytics. Also fixed a sync-conflict gap flagged by WU1's audit: two offline devices independently creating a successor for the same predecessor now resolves via last-write-wins (the losing pact's link is cleared, not the record itself) instead of the losing record silently never landing on other devices.
...
- [user] You can now pause a pact with a break instead of stopping it — showups during the break won't count as failures, and you can pick up right where you left off with a "Resume pact" action whenever you're ready. Pacts on break now show an "On break" status in the pact list instead of "Active," and a new "Break" filter chip lets you find them.
- [app] HAB-195 WU6 (final): Added the pact list's "Break" filter chip (`PactListState.breakSelected`) as an independent multi-select toggle alongside the existing status chips — not a new `PactStatus`, but Active and Break are mutually exclusive for on-break pacts specifically: `filteredEntries` is `e.onBreak ? breakSelected : activeFilters.contains(status)`, so Active alone never surfaces an on-break pact, only Break does (or both together). `PactListEntry.onBreak` is derived per active pact via a new `PactListQueryService.getBreaksForPact`; `_PactTile` now shows a distinct "On break" badge (reusing the `showupOnBreak` string) instead of "Active" for those entries. Hidden entirely when `pact_breaks_enabled` is off. Reworked `_PactFilterChipsRow` from a horizontally-scrolling `Row` to a staggered, no-scroll `Wrap` (`showCheckmark: false`) to fit the 5th chip. Reworded the pact-creation commitment gate across all 4 languages to acknowledge breaks as the one exception to "no exceptions" instead of promising "no pausing a pact." Flipped the `pact_breaks_enabled` Remote Config default from `false` to `true`, completing the pact-breaks feature end-to-end. Filled integration scenario S9. Updated `docs/PRODUCT_SPEC.md` and `docs/FEATURE_TOGGLES.md`.
- [app] HAB-195 WU6: Fixed `ShowupDetailViewModel.canStartBreak` (WU4.2) offering "Take a break" on a showup that's already on break — reachable once a break is resumed, since the showup it covered stays marked on-break permanently (by design) even though the break itself no longer blocks starting a new one. Added an `!isOnBreak` condition, computed once and shared with the existing late-open auto-fail suppression check.

## [Unreleased]

Internal-only changes (CI, tooling, tests, workflow/skill docs) that did not change the app — no `pubspec.yaml` version bump, no build, no release. See `docs/VERSIONING.md` for the rule.

- [wip] HAB-195 WU5.2: Added the "In a break" banner + "Resume pact" action on Pact Detail (iOS + Android), replacing the (now-hidden) Take a Break button whenever a break is active — shows the break's rationale and either its planned end date or "No end date yet", with the compact "Resume pact" button placed beside the text (not stacked below it) to keep the banner short. Tapping "Resume pact" opens a confirm dialog, then calls `PactBreakService.stopBreak` and fires a new `PactBreakStoppedEvent` analytics event; the banner fades/collapses out on success instead of vanishing instantly, reusing the same `AnimatedReveal` widget as the break-creation flow's end-date toggle. `PactDetailState` gained `isStoppingBreak`/`stopBreakError`. Filled integration scenario S7: resuming a break stops it at the current instant, keeps already-in-window showups on break permanently, releases later ones, and re-schedules their reminders. New `pactOnBreakTitle`/`pactBreakUntilLabel`/`resumePact*` l10n strings across all 4 locales; added "resume" to the GLOSSARY per-language canonical-terms table.
- [wip] HAB-195 WU5.1: Added the blue on-break display treatment across every read surface. New `ShowupUiState.onBreak` value (checked after done/failed, before active/waitingForStart/planned) and a matching blue `ShowupStatusColors.onBreak`, shown as a plain blue color on the dashboard calendar-strip dots, a blue pause icon + label on the daily showup-list tile, and the showup detail status badge. On the pact timeline, an on-break showup now paints its existing streak/single milestone blue instead of getting a separate entry — consecutive on-break days form a streak exactly like a done/failed run, showing the covering break's rationale in place of a note. New `showupOnBreak` l10n string across all 4 locales.
- [wip] HAB-195 WU4.2: Added the tail-zone showup entry point into the break-creation flow — a "Take a Break" button now also appears on `ShowupDetailScreen` for a showup that's auto-failed or still unresolved (not done) and within the tail-zone window (last N days, `pact_timeline_no_grouping_tail_period_in_days`), reusing the same `PactBreakCreationScreen` already wired from Pact Detail (WU4.1) with `source: 'tail_zone_showup'`. `ShowupDetailViewModel.load()` now computes `canStartBreak` (flag on, no unresolved break already blocking a new one, pact still exists) alongside the existing on-break auto-fail suppression check, sharing the same break lookup. Filled integration scenario S4: opening the flow from a past tail-zone showup still defaults the break's start date to today, leaves the triggering showup untouched, and correctly marks a later in-window showup on-break.
- [wip] HAB-195 WU4.1 polish: Separated "Take a Break"/"Stop Pact" from the read-only sections above with a divider on Pact Detail (iOS + Android). Replaced the break-creation screen's "No end date yet" toggle animation (previously `AnimatedSize` wrapping `AnimatedSwitcher`, which didn't actually stay synchronized) with a new shared `AnimatedReveal` widget (`lib/theme/widgets/`) that drives the end-date row's height and opacity from a single animation value every frame, so the toggle shift and the row's fade happen simultaneously — and, since it doesn't clip, the fading row visibly overlaps behind the toggle as it slides into place instead of looking like an opaque panel covering it.
- [wip] HAB-195 WU4.1: Added the break-creation flow (start date, end date or "until pact ends"/open-ended toggle, mandatory rationale) launched from a new "Take a Break" entry point on Pact Detail (iOS + Android). `PactBreakCreationViewModel` calls `PactBreakService.startBreak` (from WU2) and fires a new `PactBreakStartedEvent` analytics event; the break-creation screen also fires its own `PactBreakCreationAnalyticsScreen` screen-view event. `PactDetailViewModel.load()` now derives `PactDetailState.activeBreak` from the pact's already-fetched break history, gating the entry point (hidden when the feature flag is off, the pact isn't active, or a break already blocks starting a new one) — the button is grouped visually next to "Stop Pact". Filled integration scenarios S1 (fixed end), S2 (open-ended), S3 (mandatory rationale), and S8 (blocked while a break is active). New l10n strings across all 4 locales (en/de/fr/ru), including renaming the open-ended toggle label to "No end date yet" after product review. Not yet wired for the tail-zone showup entry point — that's WU4.2.
...
- [wip] HAB-195 WU1.2: Added the `pact_breaks` table (schema v5 migration in `HabitLoopDatabase`), `PactBreakMapper`, and `SqlitePactBreakRepository`/`InMemoryPactBreakRepository` implementing `PactBreakRepository`. Added the `pact_breaks_enabled` Remote Config flag (defaults `false`) wired through `RemoteConfigDefaults`/`FeatureFlags`. Not yet wired into DI or any UI — no consumer until WU2.
- [wip] HAB-195 WU1.3: Extended the pact/showup Firestore sync pattern (SyncMapper, FirestoreClient, SyncService/FirestoreSyncService — upload, flush, force-sync, pull-remote-merge) to a third entity, `PactBreak`; added `PactBreakSyncRepository`/`NoopPactBreakSyncRepository` implementing the sync interface; `SqlitePactBreakRepository` now also implements `PactBreakSyncRepository` and gained `getBreakById` method; wired `pactBreakRepositoryProvider` and `pactBreakSyncRepositoryProvider` into DI and `main.dart` (production instance uses `SqlitePactBreakRepository`, inert in-memory default for other environments); `ForceSyncResult` gained `pactBreaksFailed` counter. `pactBreakRepositoryProvider` deliberately does NOT follow the must-override-throw pattern of `pactRepositoryProvider`/`showupRepositoryProvider` — it defaults to an inert in-memory repo since nothing outside the sync layer reads pact breaks yet. Not yet used by any application service — `PactBreakService` lands in WU2.
- [wip] HAB-195 WU2: Added `BreakDerivation.isShowupOnBreak` (pure on-break predicate, `lib/domain/pact/break_derivation.dart`) and `PactStats.skippedOnBreak` (derived from break windows via `compute`/`fromCounts`). Threaded `PactBreak` through `PactDetailCache`/`PactDetailBundle` (now fetches/caches a pact's breaks alongside its showups so `skippedOnBreak` is always correct on the live read path) and `PactStatsService.buildStats`/`currentStats`/`persistStats` (the frozen `pact.stats` snapshot persisted by `persistStats` conservatively computes `skippedOnBreak: 0` until a caller supplies real breaks — no existing call site does yet — while the cache itself stays correct regardless, so no regression). Added `PactBreakService` (`startBreak`/`stopBreak`/`getActiveBreak`/`getBreaksForPact`) — validates the single-active-break rule, persists, cancels in-window reminders, write-through syncs, and evicts the pact's detail cache; wired as `pactBreakServiceProvider`. Not yet wired into any UI or the dashboard's auto-fail/notification sweeps — that's WU3.
- [wip] HAB-195 WU3: Wired `PactBreakService` into the dashboard's auto-fail/gap-fill sweeps and showup-detail late-open path — both now skip on-break showups (load active breaks per active pact at sweep time). Extended `ReminderSchedulingService.scheduleRemindersForShowups` with an optional `breaks` parameter; it now filters out showups inside an active break window. `PactBreakService.stopBreak()` (from WU2) now reschedules reminders for showups released when a break ends (a gap discovered while filling scenario S10: auto-fail resumption works "for free" since on-break state is derived at read time, but notification scheduling is a one-shot side effect). Extended `integration_test/harness.dart` with `pactBreakRepo` and a `buildBreak` fixture; filled in scenarios S5 (on-break showups not auto-failed) and S10 (reminders inside break window suppressed + restored on break end). Also folded in two minor code-style cleanups discovered during refactor: collapsed guard-clause chains into single logical expressions in `reminder_scheduling_service.dart` and `debug/ui/{android,ios}/remote_config_overrides_page_{android,ios}.dart`.
...
- [test] HAB-196: Fixed all 4 integration test failures found when `run-scenarios` was dispatched for the first time in a while (`full_wizard_creates_pact`, `rename_from_pact_detail_updates_displayed_name`, `rename_via_showup_detail_updates_shown_name`, `showup_note_write_through_visible_in_timeline_same_session`) — every one turned out to be a distinct, concrete test-authoring bug rather than the CI-hardware instability tracked separately in GitHub issue #297. `create_pact_flow_test.dart` never fixed its clock, so the wizard's Mon-Fri 08:00 schedule was evaluated against real wall-clock time and today's showup was legitimately absent whenever the suite ran after 08:00 — fixed by freezing `pactCreationTodayProvider`/`pactCreationSubmitNowProvider`/`todayProvider` to one consistent value, matching the pattern already used elsewhere. `edit_pact_flow_test.dart`'s wizard-swipe helper dragged from the PageView's exact center, which landed on the just-focused habit-name field and lost the gesture arena to text selection — fixed by mirroring `create_pact_flow_test.dart`'s working top-right-zone drag. `pact_timeline_flow_test.dart` waited for the note-save button before scrolling it into view, but `ShowupDetailContent`'s `ListView` only lazily realizes children within the viewport, and the on-screen keyboard pushed the button below the fold — fixed by replacing `waitFor`+`ensureVisible` with `tester.dragUntilVisible`. Also removed a redundant `hasActivePactsProvider` invalidation in `dashboard_screen.dart`'s `navigateToPactCreation()` that duplicated `dashboardRefreshSignalProvider`'s own listener (behaviour-preserving cleanup, not itself a fix for any of the four).
- [test] HAB-188: Fixed two integration tests (`sync_on_login_flow_test.dart`, `fresh_install_flow_test.dart`) that hardcoded Material icon assertions against the iOS dashboard's sync-status icon — a regression from WU3's Material/Cupertino icon-family fix, since these tests previously passed on iOS only because of the bug WU3 fixed. Added a platform-aware `syncIconFor` helper to the integration test harness.
- [wip] HAB-188 WU1: Deleted the dangling "Load more" pagination test stub (`integration_test/pact_timeline_flow_test.dart`) — 6 unfilled TODO steps with no driver code, for a feature (timeline pagination) that was already fully dropped from `PactTimelineService` back in HAB-116. No production code changed.
- [wip] HAB-188 WU2: Fixed Android primary-button hierarchy inconsistency — the dashboard's "create first pact" empty-state CTA and the pact-detail "save note" button both used `ElevatedButton`; changed both to `FilledButton` to match the primary/high-emphasis styling used everywhere else in the Android UI (`OutlinedButton`=secondary, `TextButton`=tertiary were already correct and untouched).
- [wip] HAB-188 WU3: Fixed iOS dashboard toolbar sync-status icon rendering as a Material icon instead of Cupertino — split `syncStatusIconData` into `syncStatusIconDataMaterial`/`syncStatusIconDataCupertino` (mirroring the existing `ShowupStatusColors.material()`/`.cupertino()` pattern) and routed each platform's dashboard to the correct variant.
- [wip] HAB-188 WU4: Split `DashboardViewModel._loadInner` (~175-line method doing 6 distinct jobs) into six named private steps (`_loadPactsContext`, `_runGapFillSweepAndScheduleReminders`, `_computeTodayIndex`, `_runAutoFailSweep`, `_assembleAndSetState`), and extracted a shared `_autoFailShowup` helper to de-duplicate the near-identical gap-fill-sweep and auto-fail-sweep try/catch-log-analytics-cancel-increment shape. Pure, behaviour-preserving refactor — the existing 1259-line test suite passed unmodified.
- [ci] HAB-192: Fixed `resolve-version` composite action failing on every real `main` build (`grep: lookbehind assertion is not fixed length`) — the build-number extraction regex used a variable-length PCRE lookbehind, which is invalid and could never have matched; replaced with a `\K` reset so the pattern matches whether or not the preceding text is fixed-length. Latent since HAB-183 WU1 (2026-07-19), only just exposed because every `main` push since then was HAB-187's `[wip]`-tagged WUs, which skip this job.
- [test] HAB-179: Re-enabled 2 previously-flaky Android CI scenarios with real timing fixes; broader CI infra instability tracked separately in a GitHub issue.
- [ci] HAB-185 WU2: Added Unreleased section validation to `lint.py` to enforce tag-classification rules on `## [Unreleased]` entries, ensuring non-app-changing entries use only `[ci]`/`[meta]`/`[test]`/`[wip]`/`[user-none]` tags and preventing misclassified `[user]`/`[app]` bullets.
- [wip] HAB-185 WU1: Gate `pubspec.yaml` version bumps on app-changing CHANGELOG entries. Reworked `ship` skill step 2 to route entries by classification tag: `[user]`/`[app]` entries get a fresh numbered `## [X.Y.Z]` heading inserted at the file top (sealing any open `## [Unreleased]` batch); non-app-changing entries append to the open batch or open a fresh one if none exists. Step 4 (version bump) becomes conditional—only runs if step 2 created a new heading. Added shared `heading_boundaries.py` helper to bound each entry's body by the next `## [...]` heading of any kind, fixing silent entry absorption in `distribute.py`/`release_notes.py`/`lint.py`. Updated `docs/VERSIONING.md` to clarify `pubspec.yaml` represents app build version, not repo commit history. Net LoC: +184 (scripts), -0 (docs).
- [meta] HAB-186: Added a `SessionStart` Claude Code hook (`.claude/hooks/session_start.sh`, matcher `startup`) that automatically injects the AGENTS.md session-start checklist (Linear MCP auth check, communication style, `summarize` skill, `checkup/due.py`) as context at the start of every new session, instead of relying on Claude to remember the manual instruction.
- [meta] HAB-187 WU0: Plan approval for the design-system + accessibility pass consolidating checkup findings CHK-2026-07-20-heavy-2/-3/-7/-8/-9. Documented the new `AppSpacing`/`AppTypography` theme classes in `ARCHITECTURE.md`. No new integration scenarios (infra/refactor exemption — verified at the widget-test level in later WUs); recorded the WU0 regression-baseline audit (63/64 integration tests green, one pre-existing flake in `pact_timeline_flow_test.dart` confirmed non-reproducible in isolation and unrelated to this ticket).
- [wip] HAB-187 WU1: Split `_PactsPanelState.build` (357-line God-widget, 6-7 levels of nesting — checkup finding `CHK-2026-07-20-heavy-2`) into four sub-widgets in `lib/slices/pact/ui/generic/pacts_summary_bar.dart`: `_PactsPanelHeader` (drag handle + counts + gesture math), `_PactFilterChipsRow`, `_PactListBody` (the sliver `CustomScrollView`), `_ArchivedPactsSection`. Behaviour-preserving — `pactsPickSnapTarget` and the `DraggableScrollableController`/outer `LayoutBuilder` wiring stay in the parent `_PactsPanelState`. Added `test/slices/pact/ui/pacts_summary_bar_test.dart` (8 widget tests covering header, filter-chip switching, list-body content, and the archived-pacts section) as a regression net, written and confirmed green against the pre-split widget before refactoring, then re-confirmed green after. Existing `pacts_panel_test.dart` suite (snap-target unit tests + header/hidden-state widget tests) untouched and stays green. Net LoC: +624/-261.
- [wip] HAB-187 WU2: Extracted `showCupertinoPickerSheet` (`lib/slices/pact/ui/ios/cupertino_picker_sheet.dart`, feature-local — not promoted to `lib/theme/` since it has only one consumer so far) from the 5 duplicated Cupertino picker-sheet scaffolds in `schedule_step_ios.dart` — checkup finding `CHK-2026-07-20-heavy-3`. Behaviour-preserving. Added `test/slices/pact/ui/ios/cupertino_picker_sheet_test.dart` (6 widget tests for the new helper, written red before implementation) and `test/slices/pact/ui/ios/schedule_step_ios_picker_sheets_test.dart` (4 widget tests exercising the daily/weekday/monthly-by-weekday/monthly-by-date picker call sites end to end, confirmed green both before and after the extraction as a regression net). Net LoC: `schedule_step_ios.dart` -105 (711→606), +38 new helper file.
- [wip] HAB-187 WU3: Added `AppSpacing` (`lib/theme/spacing.dart`, scale `xs=4/sm=8/md=12/lg=16/xl=24`) and `AppTypography` (`lib/theme/typography.dart`, named `wizardStepTitle`/`wizardHeroNumber` styles + a `CupertinoTextThemeData` wired into `HabitLoopTheme.cupertinoTheme`) — checkup findings `CHK-2026-07-20-heavy-7`/`-8`. Extracted `HabitLoopColors` out of `habit_loop_theme.dart` into `lib/theme/colors.dart` (re-exported) to break a would-be import cycle with `typography.dart`. Behaviour-preserving: every migrated `EdgeInsets`/`SizedBox` literal is replaced only where it already matched a scale value exactly, and the new `CupertinoTextThemeData` override forwards the same primary color the theme already derived implicitly (`tabLabelTextStyle` is the one exception — the app has no `CupertinoTabBar` yet, so it was safe to give it a real value to make the wiring widget-testable). Adopted in the WU1/WU2 widgets (`pacts_summary_bar.dart` and its sub-widgets, `schedule_step_ios.dart`) plus the other iOS wizard step widgets found in scope (`reminder_step_ios.dart`, `showup_duration_step_ios.dart`, `summary_step_ios.dart`, `pact_duration_step_ios.dart`, `habit_name_step_ios.dart`); `cupertino_picker_sheet.dart` had no literal matching the new scale so was left unchanged. Android widgets untouched per plan. Added `test/theme/spacing_test.dart` (scale is ascending and distinct) and `test/theme/typography_test.dart` (widget test proving `HabitLoopTheme.cupertinoTheme` resolves `AppTypography.cupertinoTextTheme`, not Flutter's implicit default). Net LoC: +62/-67 (widget migrations), +108 (new theme + test files).
- [wip] HAB-187 WU7: Renamed `AppSpacing` (`lib/theme/spacing.dart`) from the WU3 named scale (`xs/sm/md/lg/xl`) to a numeric `s`-prefixed scale where each constant is named after its own value (`s4=4`, `s8=8`, `s12=12`, `s16=16`, `s24=24`), and widened it with `s2/s6/s10/s14/s20/s32` ahead of the WU8 app-wide sweep. Mechanical rename across all 41 existing call sites (`pacts_summary_bar.dart` and the 6 iOS `*_step_ios.dart` wizard widgets migrated in WU3) — behaviour-preserving, no rendered layout change. Updated `test/theme/spacing_test.dart` for the new token names and widened scale, and the stale `xs`/`sm`/`md`/`lg`/`xl` naming reference in `docs/ARCHITECTURE.md`.
- [wip] HAB-187 WU8: App-wide `AppSpacing` token sweep — migrated 274 raw `EdgeInsets`/`SizedBox` numeric literals onto the widened `AppSpacing` scale across 40 files under `lib/` (the 6 iOS `*_step_ios.dart` wizard widgets and `pacts_summary_bar.dart` were WU7's charter and untouched here). Snapped the two approved near-miss one-offs (`7 → AppSpacing.s8`, `5 → AppSpacing.s6`) alongside exact-scale matches. Behaviour-preserving, uniform mechanical substitution — deliberately exceeds the usual file-count/LoC review caps. Left raw: `showup_status_dots.dart:62`'s `horizontal: 1` margin (snapping to `s2` would double the inter-dot spacing) and any literal that does not exactly match the scale (`0`, `28`, `44`, `110`, `216`, `200`, etc.). Also swept 22 exact-scale literals in `habit_name_step_ios.dart` and `schedule_step_ios.dart` that WU7's mechanical rename missed, since those two files were out of WU7's stated file list. Added `AppSpacing.s0 = 0` and snapped the last two raw-0 `EdgeInsets.fromLTRB` components (`wizard_page_scaffold.dart`, `remote_config_overrides_page_android.dart`) so the scale covers every spacing value in the app with no raw-literal exceptions.
- [wip] HAB-187 WU4: Dashboard accessibility pass — covers the dashboard portion of `CHK-2026-07-20-heavy-8` and the day-circle text-scaling half of `CHK-2026-07-20-heavy-9`. Calendar-strip day buttons (`dashboard_body.dart`) now carry a localized `Semantics(button: true, selected: ...)` node (date + "today" wording via new `dashboardDayToday` l10n key and `formatAccessibleDate()`), meet the Android/iOS tap-target minimum via `Expanded` + `ConstrainedBox(minHeight: 48)`, and wrap the day digit in a `FittedBox` so it no longer overflows at large system text-scale settings. Dashboard app-bar icon buttons (rcOverrides, sync status, language picker, about, kebab, create-pact FAB) now expose an accessible label — a Material `tooltip` on Android, a `Semantics` label on iOS (`CupertinoButton` has no built-in tooltip) — via a new shared `dashboardActionLabel()` helper. New l10n keys (`dashboardMoreOptionsTooltip`, `dashboardDayToday`) added to all four ARB files. Added 8 widget tests exercising `meetsGuideline(androidTapTargetGuideline/iOSTapTargetGuideline)`, `matchesSemantics(...)`, and a large-text-scale overflow check, per the WU4-6 test-rigor requirement noted in `docs/knowledge/notes/HAB-187.md`.
- [wip] HAB-187 WU5: Wizard/pact-creation accessibility pass — covers the wizard portion of `CHK-2026-07-20-heavy-8`. `WizardStepIndicator` segments (`wizard_step_indicator.dart`, previously a 4px-tall bar with no semantics) now carry a `Semantics(button: true, selected: ...)` node with a new `wizardStepIndicatorLabel` l10n key ("Step N of M") and meet the 48pt tap-target minimum via `ConstrainedBox`. `OptionTile` (schedule-type selector) now exposes `Semantics(selected: isSelected, button: true, label: ...)` reusing its existing `label` field — no new l10n key needed. `SlotScheduleEditor`'s remove-slot `IconButton`s (previously ~20×20 with no label) now carry a `tooltip` (new `scheduleRemoveSlotTooltip` key) and rely on `IconButton`'s default 48×48 tap target instead of the explicit zero-padding override; weekday-toggle buttons now carry a `Semantics(selected:, button:, label:)` node using the full localized weekday name (new `formatWeekdayName()` intl helper in `date_formatters.dart`, avoiding 7×4-locale key duplication) and meet the 48pt tap-target minimum via `ConstrainedBox`. Added/extended widget tests in `wizard_step_indicator_test.dart`, `option_tile_test.dart`, and `slot_schedule_editor_test.dart` (`matchesSemantics`, `meetsGuideline`, and scoped `tester.getRect` checks — the latter used instead of a tree-wide guideline check on `SlotScheduleEditor` since the untouched, out-of-scope `_TimeChipButton` sibling isn't yet guideline-compliant).
- [wip] HAB-187 WU9: Colour token adoption — replaced raw brand-colour-candidate literals with `HabitLoopColors` tokens, but only at **icon, decorative, and non-adaptive-to-begin-with** sites: `sync_status_handler.dart`'s `syncStatusIconColor()`, the remove-slot icon in `schedule_step_ios.dart`, the commitment-warning banner background in `habit_name_step_ios.dart`, `ShowupStatusColors.material()`'s `waitingForStart`, the auto-failed banner's background/border in `showup_detail_page_ios.dart`, `pact_timeline_page_ios.dart`'s concluded-pact status *dot* (not its outcome text), and the debug-only restart-banner color in both `remote_config_overrides_page_ios.dart`/`..._android.dart` plus Android's seed-done status text (was already a flat `Colors.green`, so no adaptivity to lose). Left every **text-color** site that depended on `CupertinoColors.*.resolveFrom(context)` (or Apple's default value) unchanged on `CupertinoColors` — an audit caught that swapping those to flat `HabitLoopColors.danger`/`success` drops dark-mode contrast to ~3.7-4.1:1 against iOS's black `systemBackground`, below the 4.5:1 text minimum (`PactStatusColors.cupertino()`, `pact_timeline_page_ios.dart`'s outcome text, `pact_edit_page_ios.dart`/`pact_detail_page_ios.dart`'s error/stop text, `showup_detail_page_ios.dart`'s mark-failed/auto-failed text, and the iOS debug seed-status text — the latter two additionally upgraded from a latent non-adaptive literal to a genuine `.resolveFrom(context)` call while reverting, since the pre-WU9 code had never actually resolved them). `ShowupStatusColors.cupertino()`'s `done`/`failed`/`waitingForStart` were excluded for the same reason and stay on `CupertinoColors`, proven by its own light/dark differential test. Also untouched: every `CupertinoColors.systemGrey` site (WU6's contrast-fix scope) and adaptive system semantics (`secondaryLabel`, `separator`, `systemBackground`, `tertiarySystemFill`, …).
- [wip] HAB-187 WU10: Typography token adoption — expanded `AppTypography` (`lib/theme/typography.dart`) with 9 new tokens (`sectionTitle`, `body`, `bodyRelaxed`, `caption`, `captionItalic`, `dateCaption`, `overline`, `emphasis`, `valueEmphasis`) covering `TextStyle`/`fontSize` literal shapes that were duplicated 2+ times across iOS, Android, and generic widgets (found by pattern-matching all 90 `TextStyle(` call sites app-wide, ignoring colour). Migrated ~30 call sites onto them across `about_info_header.dart`, the habit-name-step trio, `commitment_dialog_content.dart`, `wizard_page_scaffold.dart`, `showup_detail_content.dart`, `pact_detail_page_ios/_android.dart`, `pact_edit_page_ios/_android.dart`, `summary_row.dart`, `showup_detail_page_ios/_android.dart`, `pact_timeline_page_ios/_android.dart` (the largest single group — section-header/date/label/outcome text), the onboarding-carousel language-picker text, and `slot_schedule_editor.dart`'s time-chip button. None of the new tokens carry a colour — sites needing one apply `.copyWith(color: ...)` so every rendered colour is unchanged (colour/contrast correctness stays WU6's job). `habit_name_step.dart`'s default `titleStyle` now points at the existing `wizardStepTitle` token instead of duplicating its literal. Left as-is: single-occurrence literals, sites already driven by `Theme.of(context).textTheme.*`/`CupertinoTheme` (Android's Material title styles, the weekday-toggle chip), widget-internal styles with no cross-call-site duplication (`StatusBadge`, `SectionHeader`, `OverrideBadge`, `RestartRequiredBanner`, `DateRowTile`), and the debug Remote Config screens' internal/monospace styles.
...
## [0.48.5] — 2026-07-03 (PR #217 merged)
