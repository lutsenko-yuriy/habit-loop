...
- [test] HAB-259: fixed a latent time bomb in `pact_detail_view_model_test.dart` — its shared `_pact` fixture hardcoded `endDate: DateTime(2026, 9, 1)` with no pinned "now", so once wall-clock time caught up to that date the pact legitimately auto-completed mid-test, breaking two tests that assumed it stayed active. Pinned `pactDetailNowProvider` explicitly in both. No production code touched.
