...
- [wip] HAB-187 WU9: Colour token adoption — replaced raw brand-colour-candidate literals (`CupertinoColors.destructiveRed`/`systemRed` → `HabitLoopColors.danger`; `activeGreen`/`systemGreen`/`Colors.green` → `HabitLoopColors.success`; `systemYellow`/`systemOrange`/`Colors.amber` → `HabitLoopColors.sunrise`) across 11 files: the centralized `PactStatusColors.cupertino()`/`ShowupStatusColors.material()` mappings, `sync_status_handler.dart`'s `syncStatusIconColor()`, `pact_timeline_page_ios.dart`'s status-dot/outcome-text colors, the destructive-action sites in `pact_edit_page_ios.dart`/`pact_detail_page_ios.dart`/`schedule_step_ios.dart`/`showup_detail_page_ios.dart`, the commitment-warning banner in `habit_name_step_ios.dart`, and the debug-only restart-banner/seed-status colors in both `remote_config_overrides_page_ios.dart` and `..._android.dart`. Deliberately excluded `ShowupStatusColors.cupertino()`'s `done`/`failed`/`waitingForStart` fields: an existing test (`showup_status_colors_test.dart`) proves those colors are supposed to differ between light and dark mode, which a static `HabitLoopColors` constant can't reproduce — migrating would have silently removed real dark-mode adaptivity, so that factory stays on `CupertinoColors`. Also untouched: every `CupertinoColors.systemGrey` site (WU6's contrast-fix scope) and adaptive system semantics (`secondaryLabel`, `separator`, `systemBackground`, `tertiarySystemFill`, …). Updated the 3 existing tests that pinned the old literal colors (`pact_status_colors_test.dart`, `pact_timeline_page_ios_test.dart`, `dashboard_screen_test.dart`) to assert the new brand tokens instead. Net LoC: +50/-43.
