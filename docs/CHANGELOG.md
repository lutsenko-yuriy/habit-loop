...
- [wip] HAB-206 (WU3): `PactDetailScreen`'s main content now animates as a combined slide + fade (250ms, ease-out) whenever a chain-link tap swaps to a neighboring pact, direction-aware — navigating to the predecessor slides the outgoing content out to the right while the incoming content slides in from the left, and navigating to the successor mirrors this. New `PactDetailContentTransition` wraps the platform pages' main content, keyed by pact id. The `_isAnimating` guard from WU1 now stays set for the full visual transition (not just the data reload) so a second tap mid-animation stays a no-op. Widget-tested for both directions, hit-testing suppression on the outgoing content, and no-animation on a same-key rebuild.
