...
- [wip] HAB-195 WU5.1: Added the blue on-break display treatment across every read surface. New `ShowupUiState.onBreak` value (checked after done/failed, before active/waitingForStart/planned) and a matching blue `ShowupStatusColors.onBreak`, shown on the dashboard calendar-strip dots and the daily showup-list tile (both get a pause glyph over the blue color), the showup detail status badge, and a new per-break `PactBreakMilestone` entry (rationale + dates) on the pact timeline, merged chronologically alongside the existing showup milestones. Individual on-break showups stay invisible on the timeline, same as any other pending showup — only the break's own window gets an entry. New `showupOnBreak` l10n string across all 4 locales.
