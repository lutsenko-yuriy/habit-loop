...
- [wip] HAB-254 (WU2): added `NotificationReconciliationService` (`lib/slices/reminder/application/notification_reconciliation_service.dart`) — loads active pacts/showups/breaks, computes the desired notification set via `ReminderPlanner`, diffs it against `NotificationService.getPendingNotifications()` by id, and schedules whichever notification kind is missing / cancels any showup no longer desired. Added the `notification_reconciliation_enabled` kill-switch (default `true`) and the `notifications_reconciled` analytics event, logged only when a run actually changes something. Wired a provider (`notificationReconciliationServiceProvider`); no trigger calls `reconcile()` yet — that's WU3.
