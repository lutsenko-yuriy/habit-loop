...
- [meta] HAB-186: Added a `SessionStart` Claude Code hook (`.claude/hooks/session_start.sh`, matcher `startup|resume`) that automatically injects the AGENTS.md session-start checklist (Linear MCP auth check, communication style, `summarize` skill, `checkup/due.py`) as context at the start of every session, instead of relying on Claude to remember the manual instruction.
