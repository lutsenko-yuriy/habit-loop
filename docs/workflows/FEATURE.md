...
**When to use In QA vs Done directly after merge:** see `skills/shared/project-config.md`'s
"In QA path patterns" — the single source for this list (also used by `ship` step 1).
...
10. **Review loop** — starts automatically once the PR is open (see step 9), unless the user asked to hold; repeat until the user explicitly approves the PR:
    ...
    1. Wait for both review skills (`review-architecture`, `audit-code`), the Codecov patch-coverage report, and the user to finish leaving comments.
       - **Immediately after invoking each routed skill, confirm its subagent actually spawned** (the Agent tool call returns an `agentId`, or `ListAgents` shows it) before moving on to anything else. A skill's routing message ("route this to a subagent, spawn an Agent with model X") is an instruction to act on, not content to just read — treat the two the same way you'd treat a TODO you haven't checked off. Skipping the actual spawn silently drops that review pass with nothing visibly wrong until someone asks for a status update (HAB-258 debrief, 2026-09-06).
