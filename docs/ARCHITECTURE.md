...
│   │       ├── local_auth_service.dart            # LocalAuthService — debug/profile-only stateful fake; initialize() emits anonymous state; linkWithGoogle() upgrades to localUserId instantly; signOut() reverts to anonymous; wired when debug_backend = 'local'
