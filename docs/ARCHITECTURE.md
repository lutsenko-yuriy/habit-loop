...
└── slices/
    ...
    │   ├── application/               # PactBuilder, PactCreationState, PactStatsService, PactTransactionService, PactTimelineConfig, PactTimelineGrouper, PactTimelineEvent (sealed union), PactTimelineService, PactTimelinePage, PactTimelineCache
...
- `PactTimelineGrouper` (`slices/pact/application/`) — pure, stateless grouping algorithm operating on a list of `Showup` domain objects. Partitions showups into `PactTimelineEvent` items: streak items (single-outcome run ≥ threshold or in tail zone), group items (mixed or short run below threshold outside tail), noted-showup items (always individual), and single-showup items (lone done/failed in tail with no note). Receives `PactTimelineConfig` at construction; has no I/O dependencies — fully unit-testable.
- `PactTimelineEvent` (`slices/pact/application/`) — sealed union of six event kinds: `PactCreatedEvent`, `ShowupStreakEvent`, `ShowupGroupEvent`, `NotedShowupEvent`, `SingleShowupEvent`, `CurrentStateEvent` (active pacts) / `PactConcludedEvent` (stopped/completed pacts). Anchor events (pact created, current state / concluded) are always visible outside the paginated section.
- `PactTimelineService` (`slices/pact/application/`) — assembles a paginated timeline from the local DB. Holds a `PactTimelineCache` instance and delegates grouping to `PactTimelineGrouper`. Exposes `loadPage(pactId, pageNumber)` → `PactTimelinePage`; inserts anchor events around the paginated middle section; uses the cache to skip re-querying showups on "load more" taps.
- `PactTimelineCache` (`slices/pact/application/`) — session-scoped in-memory `Map<String, List<Showup>>` keyed by `pactId`. Cache is populated by `PactTimelineService` on the first page load; subsequent pages slice from the cached list without a DB round-trip. `evictCache(pactId)` is called from `PactStatsService.persistShowupStatus` alongside `_statsCache.remove(pactId)` so any showup mutation evicts both caches atomically.
- `PactTimelinePage` (`slices/pact/application/`) — value object returned by `PactTimelineService.loadPage`: holds the list of `PactTimelineEvent` items for the page and a `hasMore` flag.
