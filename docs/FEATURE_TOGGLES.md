...
| `pact_chaining_enabled` | `false` (HAB-202 mid-rollout; flips to `true` when WU4 ships) | "Previous Pact"/"Next Pact" links hidden from the pact detail screen and "Adjust and start again" hidden from finished pacts. Does not affect any `predecessorPactId` link already persisted — only its UI surface is gated | 2026-12-15 |
