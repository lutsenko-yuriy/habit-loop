...
| **Chain (of pacts)** | A linear sequence of pacts formed by **"Adjust and start again"** — creating a new pact pre-filled from a finished (stopped or completed) one. The new pact stores a link back to its **predecessor**; the predecessor gains a link forward to its **successor**. Each pact has at most one successor — no branching for now. A new pact's default habit name is the chain's root pact name plus a version suffix (e.g. "Vibe coding (v2)"). Stats and streaks are never shared across a chain (HAB-202). Navigated via the pact detail screen's chevron **navigation stripe** — an in-place, animated `‹ predecessor / current / successor ›` strip (HAB-206). |
